...
\date{4:00 AM, September 17th 2026}
...
\hypersetup{
	...
	urlcolor=cyan,
	citecolor=blue,
	pdftitle={m52.skripta},
	pdfauthor={James Barnard}
}
...
\begin{document}
	...
	Mocniny $i$ se cyklicky opakují ($n\in\mathbb{Z}$):
	$$i^1=i=i^{4n-3}$$
	$$i^2=-1=i^{4n-2}$$
	$$i^3=-i=i^{4n-1}$$
	$$i^4=1=i^{4n}$$
	...
	Reálná čísla znázorňujeme na reálné ose. Komplexní číslo $z = a + bi$ můžeme znázornit jako bod $(a,b)$ v rovině s kartézskými souřadnicemi. V takovém případě mluvíme o \textbf{komplexní rovině}. Horizontální osa roviny se nazývá \textbf{reálná osa} a odpovídá hodnotám Re $z$. Vertikální osa roviny se nazývá \textbf{imaginární osa} a odpovídá hodnotám Im $z$.
	...
	Vyjádření $z = r(\cos\alpha + \sin\alpha\cdot i)$ nazýváme \textbf{goniometrický tvar} komplexního čísla $z \neq 0$.
	
	\begin{itemize}
		\item číslo $r$ nazýváme \textbf{absolutní hodnota} čísla $z$ a označujeme jej $|z|$,
		\item úhel $\alpha$ nazýváme \textbf{argument} komplexního čísla $z$ a označujeme jej $\arg z$.
	\end{itemize}
	...
	\textbf{Komplexní exponenciálu} definujeme pomocí Eulerovy formule
	...
	Dokázali jsme tak $|z + w|^2\le(|z| + |w|)^2$ a po odmocnění dostáváme trojúhelníkovou nerovnost pro komplexní čísla. Pomocí geometrického významu sčítání komplexních čísel trojúhelníkovou nerovnost snadno nahlédneme z obrázku.
		...
		\begin{figure}[htbp]
		\centering
		\includegraphics[width=0.4\textwidth]{Screenshot 2026-09-18 205004.png}
	...
	\subsection{Mocniny, součet a součin imaginární jednotky}
	
	\begin{enumerate}[label=\textbf{\arabic{subsection}.\arabic*.}]
		\item Určete hodnotu následujících mocnin imaginární jednotky:
		\begin{tasks}(7)
			\task $i^3$
			\task $i^{13}$
			\task $i^{23}$
		...
		\begin{tasks}(4)
			\task $i^3 + i^{-13} - i^{-9}$
			\task $i\cdot i^{2}\cdot i^{3}\cdot i^{4}$
			\task $i^{5}\cdot i^{6}\cdot i^{7}\cdot i^{8}$
			\task $i\cdot i^{2} {\cdot} \dots {\cdot} i^{48}\cdot i^{49}$
		\end{tasks}
	...
	\subsection{Postup a řešení}
	
	{\small
		\noindent\textbf{Úloha 1.1}\\
		Mocniny $i$ určujeme na základě periody 4 (viz~\ref{sec:1.2}):
		\begin{multicols}{2}
			\begin{itemize}[leftmargin=*, itemsep=2pt]
				\item[\textbf{a)}] $i^3 = i^{4-1} = -i$
				\item[\textbf{b)}] $i^{13} = i^{4\cdot4-3} = i^{4-3} = i$
				\item[\textbf{c)}] $i^{23} = i^{4\cdot6-1} = i^{4-1} = -i$
				\item[\textbf{d)}] $i^{43} = i^{4\cdot11-1} = i^{4-1} = -i$
				\item[\textbf{e)}] $i^0 = 1$ (z definice nulté mocniny),\\ nebo $i^{4\cdot0} = i^{4} = 1$
				\item[\textbf{f)}] $i^{-2} = \frac{1}{i^2} = \frac{1}{i^{4-2}} = \frac{1}{-1} = -1$
				\item[\textbf{g)}] $i^{-9} = i^{4\cdot(-2)-1} = i^{4-1} = -i$
			\end{itemize}
		\end{multicols}
				...
				\noindent\textbf{Úloha 1.2}\\
				Při součtu nejprve převedeme na základní tvar:
				\begin{itemize}[leftmargin=*, itemsep=8pt]
					\item[\textbf{a)}] $i^3 + i^{-13} - i^{-9} = i^{4-1} + i^{4\cdot(-3)-1} - i^{4\cdot(-2)-1} = -i + (-i) - (-i) = -i$
					\item[\textbf{b)}] $i \cdot i^2 \cdot i^3 \cdot i^4 = i^{1+2+3+4} = i^{10} = i^{4\cdot3-2} = -1$
					\item[\textbf{c)}] $i^5 \cdot i^6 \cdot i^7 \cdot i^8 = i^{4}(i \cdot i^2 \cdot i^3 \cdot i^4) = i^{4}\cdot(-1) = -1$
					\item[\textbf{d)}] Pomocí úloh b) a c) můžeme odvodit: $i\cdot i^{2} {\cdot} \dots {\cdot} i^{48}\cdot i^{49} = (i\cdot i^{2} {\cdot} \dots {\cdot} i^{48})\cdot i^{49} =\\ = i^{4\cdot0}(i \cdot i^2 \cdot i^3 \cdot i^4)\cdot i^{4\cdot1}(i \cdot i^2 \cdot i^3 \cdot i^4){\cdot}\dots{\cdot}i^{4\cdot11}(i \cdot i^2 \cdot i^3 \cdot i^4)\cdot i^{49} = (i \cdot i^2 \cdot i^3 \cdot i^4)^{12}\cdot i^{49} = (-1)^{12}\cdot i^{49} =\\ = 1\cdot i^{4\cdot13-3} = i$\\
					nebo: $i\cdot i^{2} {\cdot} \dots {\cdot} i^{48}\cdot i^{49} = i^{1+2+\dots+48+49} = i^{\left(\sum\limits_{k=1}^{49} k\right)} = i^{1225} = i^{4\cdot307-3} = i$\\					
					$\sum\limits_{k=1}^{49} k$ lze vypočítat pomocí rovnice pro aritmetickou posloupnost $S_n$.
